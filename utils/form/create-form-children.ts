...
export default function createFormChildren (parentProps: FactoryPropsTypes, emit: EmitType, componentsMap: ComponentsMap): VNodeChild | undefined {
    const rootSchema = parentProps.schema
    const requiredProperties = rootSchema.required || []
    const parents = parentProps.parents

    if (!rootSchema.properties) { return h('') }
    ...
    return Object.entries(rootSchema.properties)
        ...
        .map((property) => {
            ...
            if (schema.widget === 'choice-multiple-expanded' || schema.widget === 'choice-expanded') {
                ...
                const component = isMultiple && componentsMap.selectMultipleExpanded
            ...
            if (
                ...
                if (schema.widget === 'joined') {
                    ...
                    props['onUpdate:modelValue'] =
                        (value: string) => emit('update:modelValue', { ...parentModelValues, [key]: value.split(',') })
                ...
                if (type === 'boolean' || type === 'checkbox') {
                    ...
                    props.type = 'string'
                    props['onUpdate:modelValue'] =
                    (value: any) => emit('update:modelValue', { ...parentModelValues, [key]: Number.parseFloat(value) })
